            ...
            if (events) {
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    if ((ev['@type'] === 'state') && (ev.state === 'ended')) {
                        ...
                        var btoken = `${this.session.getBearer()}`;
                        var clink = `${this.chatLink}`;
                        request({
                          url: "https://LIVEENGAGE_URL/liveengage?sender=liveengage&text="+ev.text,
                          method: "POST"
                          ...
                          if (err) {
                              ...
                              const options = {
                                    method: 'POST',
                                    url: clink+'/events.json?v=1&NC=true',
                                    headers: {
